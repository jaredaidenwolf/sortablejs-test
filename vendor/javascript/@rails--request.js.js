class FetchResponse {
  ...
  async activeScript() {
    if (!this.isScript)
      return Promise.reject(
        new Error(
          `Expected a Script response but got "${this.contentType}" instead`,
        ),
      );
    {
...
function stringEntriesFromFormData(t) {
  return [...t].reduce(
    (t, [e, n]) => t.concat(typeof n === 'string' ? [[e, n]] : []),
    [],
  );
}
...
class FetchRequest {
  ...
  async perform() {
    ...
    const t =
      this.responseKind === 'turbo-stream' && window.Turbo
        ? window.Turbo.fetch
        : window.fetch;
    const e = new FetchResponse(await t(this.url, this.fetchOptions));
    if (e.unauthenticated && e.authenticationURL)
      return Promise.reject((window.location.href = e.authenticationURL));
    e.isScript && (await e.activeScript());
    const n = e.ok || e.unprocessableEntity;
    n && e.isTurboStream && (await e.renderTurboStream());
    return e;
  }
  addHeader(t, e) {
  ...
  get formattedBody() {
    const t = Object.prototype.toString.call(this.body) === '[object String]';
    const e = this.headers['Content-Type'] === 'application/json';
